...
function LogIn({setUser}) {
  ...
  async function handleSubmit(event) {
    ...
    const res = await fetch('http://127.0.0.1:12345/api/Tokens', {
      'method': 'post',
      'headers': {
    ...
    if (res.status !== 200) {
      ...
      userDetails.token = await res.json();
        const respond = await fetch('http://127.0.0.1:12345/api/Users/' + userDetails.username, {
        'headers': {
        'Content-Type': 'application/json',
